...
function Popup({ id, setIsModalOpen }) {
  ...
  if (!image)
    return (
      <div className="bg-black/80 fixed top-0 w-full h-full z-100">
        {" "}
        <div className="flex w-[1024px] h-[723px] justify-center items-center bg-white dark:bg-[#232323] mx-auto">
          <span className="font-bold text-3xl tracking-widest text-black dark:text-white">
            Loading...
          </span>
        </div>
      </div>
      ...
      <div className="cursor-pointer shadow-md rounded-xl m-5 max-w-5xl mx-auto mt-15 bg-white">
        <div className="flex justify-center items-center h-[500px] relative">
          ...
          <div className="flex flex-row justify-between items-center p-3 absolute bottom-0 w-full">
            ...
            <button className="bg-green-600 px-4 py-2 text-xs font-bold text-white">
              Download Image
            </button>
          </div>
          <AiOutlineClose
            className="absolute cursor-pointer -top-3 -right-3 z-200 text-[#4F4F4F] w-[24px] h-[24px] rounded-full bg-[#FFFFFFCC] p-1"
